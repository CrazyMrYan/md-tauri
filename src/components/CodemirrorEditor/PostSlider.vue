...
// 获取特定目录的文档
const getPostsByDirectory = (directoryId: string) => {
...
// 添加从目录右键菜单添加文档的函数
const addPostToDirectory = (directoryId: string) => {
...
// 重命名目录对话框设置
const showRenameDirectoryDialog = (dirId: string) => {
...
// 添加子目录对话框设置
const showAddSubdirectoryDialog = (parentId: string) => {
...
const startDelDirectory = (dirId: string) => {
...
const handleDirectoryClick = (dirId: string, event: MouseEvent) => {
  // 忽略右键点击
...
<template>
  <div
    class="overflow-hidden bg-gray/20 dark:bg-[#191c20] relative"
    :class="{
  ...
  >
    <nav
      class="space-y-1 h-full overflow-auto border-r-2 border-gray/20 p-2 select-none"
      :class="{
    ...
    >
      ...
      <Dialog v-model:open="isOpen">
        <DialogTrigger as-child>
            <Button variant="outline" class="flex-1" size="xs">
            <Plus /> 新增文档
          </Button>
        </DialogTrigger>
        ...
        <Dialog v-model:open="isOpenDirDialog">
          <DialogTrigger as-child>
            <Button variant="outline" size="xs" class="flex-1" @click="openNewDirectoryDialog">
      ...
      <AlertDialog v-model:open="isOpenDelPostConfirmDialog">
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel :disabled="isDeletingPost">取消</AlertDialogCancel>
            <AlertDialogAction 
              @click="delPost()" 
      ...
      <AlertDialog v-model:open="isOpenDelDirConfirmDialog">
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel :disabled="isDeletingDirectory">取消</AlertDialogCancel>
            <AlertDialogAction 
    ...
    <div
      ...
      class="absolute top-0 right-0 h-full w-1 cursor-ew-resize hover:bg-primary/60 active:bg-primary z-10"
